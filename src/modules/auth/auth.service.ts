...
export class AuthService {
  ...
  // 3. تجهيز البيانات (partial update)
  ...
  // 4. التأكد أن هناك بيانات للتحديث
  if (Object.keys(data).length === 0) {
    throw new BadRequestException('No data provided');
  }

  // 5. التحقق من عدم وجود تغيير فعلي
  const noChange =
    (!dto.fullName || dto.fullName === user.fullName) &&
    (!dto.phoneNumber || dto.phoneNumber === user.phoneNumber) &&
    (!dto.locationName || dto.locationName === user.locationName) &&
    (!dto.latitude || dto.latitude === user.latitude) &&
    (!dto.longitude || dto.longitude === user.longitude) &&
    !profileImageUrl;
